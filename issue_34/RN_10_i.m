function [data, T_data, T_v_data, T_wB_data,...
    ...
    SW1 = 0; % 0 for counter-current, 1 for co-current 
    SW2 = 0; % 0 for linked streams, 1 for non-linked streams 
    ...
    T_in = [100, 100;               % stream A, stream C; temperature, K
            200, 200];             % stream B, stream D; temperature, K
    h_in = hcalc(T_in, p_in, fluid); % convert input to h     
    T_wA_init = [150, 150]; % initial wall temperature (left, right), K, -   
    T_wB_init = [100, 200]; % initial wall temperature (top, bottom), K, - 
    T_v_init = [150, 150]; % initial wall tail temperature (left, right), K, - 
    
    % ************************ RADIATION DATA *****************************
    T_ext_init = 300; % External body temperature, K 
    sigma = 5.676e-8; % Stefan-Boltzmann constant, W/m^2 * K^4
    As = 8; F = 1; % for He-Wall radiation 
    ...
    for k = 2 : t + 1
        ...
        % get initial p data for B stream 
        % (if streams are linked AND are counter-current)
        % (for time step only)
        if k == 2
            if SW1 == 0 && SW2 == 0
                sol = solve_p(k, 2);                   % solve right side
                p_in(2, 1) = sol(1, 2);                % D_out -> B_in
            end 
        end
        ...
        for ii = 1 : no   
            ...
            % calculate delta_p
            delta_p(1) = sol(2, 1) - sol(1, 1);         % top stream  
            delta_p(2) = sol(2, 2) - sol(1, 2);         % bottom stream
            
            % update p_in (if streams are linked)
            if SW2 == 0 
                if SW1 == 0                                    % counter-current
                    p_in(1, 2) = sol(HX_slices, 1) + delta_p(1);  % A_out -> C_in
                    p_in(2, 1) = sol(1, 2) - delta_p(2);          % D_out -> B_in
                elseif SW1 == 1                                % co-current
                    p_in(1, 2) = sol(HX_slices, 1) + delta_p(1);  % A_out -> C_in
                    p_in(2, 2) = sol(HX_slices, 2) + delta_p(2);  % B_out -> D_in
                end
            end
        end
        
        % get initial h data for B stream 
        % (if streams are linked AND are counter-current)
        % (need to do for all time steps) 
        if SW1 == 0 && SW2 == 0
            sol = solve_h(k, 2);                            % solve right side
            h_in(2, 1) = sol(1, 2);                         % D_out -> B_in
        end 
        ...
        for ii = 1 : no
            ...
            % update h_in (if streams are linked)
            if SW2 == 0 
                if SW1 == 0                               % counter-current
                    h_in(1, 2) = sol(HX_slices, 1);         % A_out -> C_in
                    h_in(2, 1) = sol(1, 2);                 % D_out -> B_in
                elseif SW1 == 1                           % co-current
                    h_in(1, 2) = sol(HX_slices, 1);         % A_out -> C_in
                    h_in(2, 2) = sol(HX_slices, 2);         % B_out -> D_in
                end
            end
    ...
    if SW1 == 0  % for counter-current, flip p_delta
        p_delta_data(:, 2, :, :) = - p_delta_data(:, 2, :, :);
    end
    ...
    function sol = solve_p(k, ii) % p solver
        ...
        % get p distribution 
        if SW1 == 0     % counter-current
            sol = [p_dist(delta_p_a, p_in(1, ii))', ...
                fliplr(p_dist(delta_p_b, p_in(2, ii)))'];
        elseif SW1 == 1     % co-current
            sol = [p_dist(delta_p_a, p_in(1, ii))', ...
                (p_dist(delta_p_b, p_in(2, ii)))'];
        end
    end
    ...
    if SW1 == 0 && SW2 == 0
        % Pressure across length plot: streams A - C
        figure
        hold on
        plot(1 : 2 * HX_slices, ...
            [squeeze(p_data(:, 1, 2 : t + 1, 1))' ...
            squeeze(p_data(:, 1, 2 : t + 1, 2))'], 'r')
        xlabel('Slices')
        ylabel('Pressure')
        title('Stream A - C')
    
        % Pressure across length plot: streams B - D
        figure
        hold on
        plot(1 : 2 * HX_slices, ...
            [squeeze(p_data(:, 2, 2 : t + 1, 1))' ...
            squeeze(p_data(:, 2, 2 : t + 1, 2))'], 'b')
        xlabel('Slices')
        ylabel('Pressure')
        title('Stream B - D')
    end
