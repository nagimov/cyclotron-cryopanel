function [Q, data, p_a_data, p_b_data, T_a_sol, T_b_sol, T_w_sol] = RN_08_e 
    %With radiative heat transfer, k_x, U, delta_p
    clc; clear;
    close all;
    tic
    ...
    p_a_in = 1.25 * 101325;  % inlet pressure of stream A, Pa
    ...
    p_b_in = 1.75 * 101325;  % inlet pressure of stream B, Pa
    ...
    T_nom = 150;
    p_nom = 101325; 
    delta_p_nom = 101325 / 2; 
    nom_values = {'helium', nom_calc(fluid_a);...
                    'nitrogen', nom_calc(fluid_b)};
    ...
    function p = p_calc(h, p, p_in, fluid)
        ...
        delta_p = delta_p_nom * (m / m_nom)^1.8 * (mu_nom / mu)^(-0.2) ...
            * (rho_nom / rho);
        delta_p = delta_p / HX_slices; %per slice 
