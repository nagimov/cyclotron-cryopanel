function [data, T_a_sol, T_b_sol, T_w_sol] = RN_06_e %With temperature distribution in the wall, ode45  
    clc; clear;
    close all;
    tic
    ...
    function dudt = dudt(h, h_prev, p, fluid)
		dudt = prop_uhp(h, p, fluid, lib) - prop_uhp(h_prev, p, fluid, lib); 
    end
	...
	function [x, fval, exitflag] = solve_h(k)  % h solver
	    ...
	    function F = eqgen(sol)
	    	...
	    	dudt_a = zeros(1, HX_slices);
	    	dudt_b = zeros(1, HX_slices);
			...
			for i = 1 : HX_slices  % slicing loop                
                Q_cond_aw(i) = Q_cond(h_a(i), p_a(i), T_w_a(i), fluid_a);
                Q_cond_bw(i) = Q_cond(h_b(i), p_b(i), T_w_b(i), fluid_b);
                dudt_a(i) = dudt(h_a(i), h_a_prev(i), p_a(i), fluid_a);
                dudt_b(i) = dudt(h_b(i), h_b_prev(i), p_b(i), fluid_b);
	...
	function sol = solve_T(i,k)  % T solver                         
        ...
        Q_cond_bw = Q_cond(h_b(i), p_b(i), T_w_prev(i,Wall_slices), fluid_b);       
