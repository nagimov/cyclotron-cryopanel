function [data, p_a_data, p_b_data, T_a_sol, T_b_sol, T_w_sol, T_w3_data, Q] = RN_09_a 
    %With wall split, movable node 
    clc; clear;
    close all;
    tic
    ...
    figure
    hold on
    plot(1:HX_slices, squeeze(T_w3_data(:,1,:)),'g')
    plot(1:HX_slices, squeeze(T_w3_data(:,10,:)),'r')
    xlabel('Slices')
    ylabel('Temperature')
    title('Wall Tail Plot')
    
    figure
    plot(1:HX_slices, squeeze(T_w_sol(:, W1, :)), 'b')
    xlabel('Slices')
    ylabel('Temperature')
    ...
    time = [1+1, round(t/4), round(t/2), t+1]; % which times to plot
    colormap autumn 
	...
	function sol = solve_T(i,k)  % T solver, for a fixed HX_slice                          
	    ...
	    function dTdt_w = eqgen(~,T_w)
            ...
            Q_cond(W2) =  K_w(W2)/b_x * (T_w(W2) - T_w(W2-1)) - Q_cond_bw;
