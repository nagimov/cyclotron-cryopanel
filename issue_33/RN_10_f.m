function [data, T_data, T_v_data, T_wB_data,...
    ...
    delta_p_nom_data = {'helium', p_atm / 50; ... 
                        'nitrogen', p_atm / 50}; % nomial pressure drop, Pa
    ...
    for ii = 1 : no
        ...
        % Grab delta_p_nom
        delta_p_val(1, :) = nom_calc(fluid{1, ii});
        delta_p_val(2, :) = nom_calc(fluid{2, ii});
        ...
        p_0(:, 1, ii) = p_dist(delta_p_val(1, 2), p_in(1, ii));
        p_0(:, 2, ii) = p_dist(delta_p_val(2, 2), p_in(2, ii));
    ...
    for k = 2 : t + 1
        for ii = 1 : no
            ...
            for i = 1 : HX_slices
                ...
                %stack in time
                data(i, 2 : N - 1, k, ii) = sol(end, 1 : WA2); % main wall
                T_v_data(i, :, k, ii) = sol(end, WA2 + 1 : WA3); % wall tail
            end
    ...
    function values = nom_calc(fluid)
        ...
        % Grab nominal values that we already stores
        ...
        % Calculate new nominal values and add them in 
    ...
    function Q_rad = Q_rad1(h, p, T_w, fluid)
        T = propsc_thp(h, p, fluid);
        [~, rad_length] = size(T_w); 
    ...
    function delta_p = p_calc(h, p, p_in, fluid)
        ...
        h_mean = mean(h);
        p_mean = mean(p);
        [~,mu,rho,~] = propsc_LmurhoPr_hp(h_mean, p_in, fluid);
    ...
    function sol = solve_p(k, ii) % p solver
        ...
        delta_p_a = p_calc(h_a_prev, p_a_prev, p_in(1, ii), fluid{1, ii});
        delta_p_b = p_calc(h_b_prev, p_b_prev, p_in(2, ii), fluid{2, ii});
    ...
    function [x, fval, exitflag] = solve_h(k, ii)  % h solver
        ...
        sol_guess = [h_a_prev(:, ii), h_b_prev(:, ii)];
	    ...
	    function eps = eqgen(sol)
            ...
            Q_rad_aw = Q_rad1(h_a, p_a, T_w_prev, fluid{1, ii});
    ...
    function sol = solve_T_wA(i, k, ii)  % Tw_A solver, for a fixed HX_slice                          
        ...
        % calculate Q from h 
        Q_cond_aw = Q_cond(h_a(i, ii), p_a(i, ii), T_wA_prev(i, 1), fluid{1, ii});   
        Q_cond_bw = Q_cond(h_b(i, ii), p_b(i, ii), T_wA_prev(i, WA2), fluid{2, ii});       
	    ...
	    function dTdt_wA = eqgen(~,T_wA)
            ...
            Q_rad_l = Q_rad2(h_a(i, 1), p_a(i), T_wA, fluid{1, 1}); % to left stream
            Q_rad_r = Q_rad2(h_a(i, 2), p_a(i), T_wA, fluid{1, 2}); % to right stream 
    ...
    function sol = solve_T_wB(i, k, ii)  % Tw_B solver, for a fixed HX_slice                          
	    ...
	    function dTdt_wB = eqgen(~,T_wB)
            ...
            if ii == 2 
                Q_rad_l = Q_rad2(h_l(i), p_l(i), T_wB, fluid{ii, 1});
                Q_rad_r = Q_rad2(h_r(i), p_r(i), T_wB, fluid{ii, 2});
