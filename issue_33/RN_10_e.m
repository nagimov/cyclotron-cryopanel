function [data, T_data, T_v_data, T_wB_data, p_data] = RN_10_e
    % Added radiative fluxes 
    clc; clear;
    close all;
    tic
    
    % ************************** COOL PROP ********************************
    CP_dump = 15; % number of time steps before we dump & re-load the library 
    ...
    As = 1; F = 1; % for He-Wall radiation 
    ...
    function Q_rad = Q_rad1(h, p, T_w, fluid)
        ...
        % sum fluxes due to all walls across all rows 
        Q_rad = sum(Q_rad, 2); 
    end
    ...
    function sol = solve_T_wA(i, k, ii)  % Tw_A solver, for a fixed HX_slice                          
        ...
        % calculate Q from h 
        Q_cond_aw = Q_cond(h_a(i, ii), p_a(i), T_wA_prev(i, 1), fluid{1, ii});   
        Q_cond_bw = Q_cond(h_b(i, ii), p_b(i), T_wA_prev(i, WA2), fluid{2, ii});       
    ...
    function sol = solve_T_wB(i, k, ii)  % Tw_B solver, for a fixed HX_slice                          
	    ...
	    function dTdt_wB = eqgen(~,T_wB)
            ...
            if ii == 2 % only worry about radiation for the bottom wall 
