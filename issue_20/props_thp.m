function eval =  props_thp(h,p,Name,lib) 
...
if size(h) == size(p)   %Check to ensure input matrices are the same size
    ...
    error('Size of input matrices for h and p must be the same')
end 


% ACTUAL PROP FUNCTION 
function f = prop_thp(h,p,Name,lib)
...
elseif strcmp(lib,'CP') == 1
    try 
        f = CoolProp.PropsSI('T','H',h,'P',p,Name);
    catch ME
    end
end    
